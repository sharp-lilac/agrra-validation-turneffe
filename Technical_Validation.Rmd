...
# Setup. Load in necessary packages, set color palette, and download data from Dryad.
...
library(vegan) #for rarefraction curve
library(factoextra) # Nice PCA biplots
library(glmmTMB)# GLMM Models with fixed and random effects, nesting, and interactions
library(car) #functions to accompany glmmTMB (e.g., Anova)
library(kableExtra) #customize tables

palette = c("#E69F00", "#56B4E9", "#009E73", "#F0E442", "#CC79A7", "#9999FF","#33A02C", "#FB9A99", 
...
organism_data_pre = dfs$Ref_Organisms_Benthic
collectors_data = read.csv("Collectors_Turneffe_V4.csv", 
...
We can create a rarefraction curve of total coral species expected to be found by number of samples. Each sampled site is plotted as a line, with localities being labelled, and reef types being indicated by color (yellow being backreef, blue being deep forereef, and green being shallow forereef). 

```{r rarespecies_benthicpim, fig.height=7, fig.width=12}

##BETTER X-AXIS

#Ensure data is only for organisms with species classification 
master_data_species = master_data %>%
  inner_join(x = master_data, y = organism_data_pre, by = "Organism")%>%
  filter(!is.na(Species))%>%
  filter(Species != "")

#Format data for vegan
data_vegan_overall = master_data_species %>%
  group_by(Site, Species) %>%
  summarise(Count = n()) %>%
  select(Site, Species, Count) %>%
  pivot_wider(names_from = Species, values_from = Count, values_fill = 0)
rownames_overall_vegan = data_vegan_overall$Site
data_vegan_overall = data_vegan_overall[,-1]
rownames(data_vegan_overall) = rownames_overall_vegan

#Create accompanying metadata
metadata = master_data_species %>%
  group_by(Site, Protocol) 

#Fetch data for curve
C = rarecurve(data_vegan_overall, step = 50, tidy = TRUE)

#Fetch data on reef types on localities based on site and add to curve data
reef_typesC = C %>%
  distinct(Site) %>%
...
C = left_join(C, reef_typesC, by = "Site")

#Determine where each rarecurve line will end, so labels can be placed properly
last_pointsC = C %>%
  group_by(Site) %>%
  slice_tail(n = 1)

#Create plot theme for rarecurve
xy =   theme(legend.position = "top", axis.text.x = element_text(angle = 0, face = "bold", size = 12, 
                                                            vjust = 0.5, hjust = 1),
        legend.text = element_text(size = 12),  
        legend.title = element_text(size = 14),
        title = element_text(size = 15), strip.text = element_blank(), strip.background = element_blank())
h_line1 = geom_hline(yintercept = c(10, 20, 30, 40), linetype = "dashed", color = "darkgrey", size = 0.75) 
h_line2 = geom_hline(yintercept = c(5, 15, 25, 35, 45), linetype = "dotted", color = "darkgrey", size = 0.5) 
v_line1 = geom_vline(xintercept = seq(0, 1500, by = 250), linetype = "dotted", color = "darkgrey", size = 0.5) 

RareC = ggplot(C, aes(x = Sample, y = Species, group = Site, color = Type)) + h_line1 + h_line2 + v_line1 +
  geom_line(size = 0.8) +
  theme_classic() +
  scale_color_manual(name = "Reef Type", 
                    values = palette[1:3]) +
  coord_cartesian(ylim = c(0, 45)) +
  geom_label_repel(data = last_pointsC, aes(label = Locality, x = Sample, y = Species),
                   label.padding = unit(0.1, "lines"), fill = "white", color = "black",
                   box.padding = unit(0.4, "lines"),
                   segment.size = 0.7,
                   force = 20, size = 6) + xy+
  scale_x_continuous(breaks = seq(0, 1500, by = 250))

#ggsave("fig2_rarefactions.png", plot = RareC, width = 14, height = 8, units = "in", dpi = 800)
RareC
```
...
ggplot(summary_data, aes(x = factor(Collector, levels = collector_order), y = Percentage, fill = Protocol)) +
  ...
  geom_col(width = 1, color = "black") + theme_classic() +
...
organism_data = organism_data_pre%>%
  select(Organism, Bucket, Bucket2, AGRRA_Bucket)%>% distinct()
...
#Theme for plot
xy =   theme(legend.position = "top", axis.text.x = element_text(angle = 90, face = "bold", size = 12, 
                                                            vjust = 0.5, hjust = 1),
        legend.text = element_text(size = 12),  
        legend.title = element_text(size = 14),
        title = element_text(size = 15), strip.text = element_blank(), strip.background = element_blank())
...
A = ggplot(filter(summary_data9, Organism %in% tail(code_orderall, 8)), 
                             aes(x = factor(Organism, levels = tail(code_orderall, 8)),
                                 y = Percent_of_Transect, fill = Protocol)) + h_line1 + h_line2 +
                      geom_boxplot(color = "black", width = 1) +  theme_classic()+
                      labs(x = "", y = "Percent Observations per Transect", title = "A") +
                      scale_fill_manual(values = palette[1:2], name = "Protocol") + xy +
                      coord_cartesian(ylim = c(0, 100)) + facet_wrap(~Protocol)

B = ggplot(filter(summary_data6, Bucket %in% tail(code_order, 8)), 
                             aes(x = factor(Bucket, levels = tail(code_order, 8)),
                                 y = Percent_of_Transect, fill = Protocol)) + h_line1 + h_line2 +
                      geom_boxplot(color = "black", width = 1) +  theme_classic()+
                      labs(x = "", y = "", title = "B") +
                      scale_fill_manual(values = palette[1:2], name = "Protocol") + xy +
...
C = ggplot(filter(summary_data7, Bucket2 %in% tail(code_order2, 8)), 
                             aes(x = factor(Bucket2, levels = tail(code_order2, 8)),
                                 y = Percent_of_Transect, fill = Protocol)) + h_line1 + h_line2 +
                      geom_boxplot(color = "black", width = 1) +  theme_classic()+
                      labs(x = "", y = "", title = "C") +
                      scale_fill_manual(values = palette[1:2], name = "Protocol") + xy +
...
D = ggplot(filter(summary_data8, AGRRA_Bucket %in% head(code_orderAGRRA, 8)), 
                             aes(x = factor(AGRRA_Bucket, levels = rev(head(code_orderAGRRA, 8))),
                                 y = Percent_of_Transect, fill = Protocol)) + h_line1 + h_line2 +
                      geom_boxplot(color = "black", width = 1) +  theme_classic()+
...
A = ggplot(filter(summary_data3_collector2_agrra, Bucket2 %in% tail(code_order2, 8)), 
                             aes(x = factor(Bucket2, levels = tail(code_order2, 8)),
                                 y = Percent_of_Transect, fill = Collector)) + h_line1 + h_line2 +
                      geom_boxplot(color = "black", width = 1) +  theme_classic()+
                      labs(x = "", y = "", title = "A") +
                      scale_fill_manual(values = palette[c(1,5,6)], name = "Collector") + xy +
                      coord_cartesian(ylim = c(0, 100)) + facet_wrap(~Collector)+
...
B = ggplot(filter(summary_data3_collector2_mbrs, Bucket2 %in% tail(code_order2, 8)), 
                             aes(x = factor(Bucket2, levels = tail(code_order2, 8)),
                                 y = Percent_of_Transect, fill = Collector)) + h_line1 + h_line2 +
                      geom_boxplot(color = "black", width = 1) +  theme_classic()+
                      labs(x = "", y = "", title = "B") +
                      scale_fill_manual(values = palette[c(2,3,4,5,6,7,8)], name = "Collector") + xy +
                      coord_cartesian(ylim = c(0, 100)) + facet_wrap(~Collector)+
  theme(strip.text = element_blank(), strip.background = element_blank())+
  theme(legend.position = "none")
#Plot legend
legend_plot = ggplot(filter(summary_data3_collector2, Collector %in% c("BOYO", "CECA", "EDBA", "JASA", "KENO", "NIMA", "TABA")), 
...
pca_data =summary_data3_collector2 %>%
  ...
  pivot_wider(names_from = Bucket2, values_from = Percent_of_Transect, values_fill = 0)%>% ungroup()
...
loadings_df = data.frame(Loading_PC1 = (loadings_pc1),
                         Loading_PC2 = (loadings_pc2))
loadings_df = loadings_df %>%
  mutate(Loading_PC1 = format(round(Loading_PC1, 3), nsmall = 3),
...
A = ggplot() +
    theme_classic() + xy +h_line1 + h_line2 + h_line3+v_line1+
    stat_ellipse(data = pca_data_metadata_supplemented,
...
B = ggplot() +
  theme_classic() + 
  xy + 
  h_line1 + h_line2 + h_line3 + v_line1 +
  stat_ellipse(data = filter(pca_data_metadata_supplemented, Collector %in% c("BOYO", "CECA", "EDBA", "JASA", "KENO", "NIMA", "TABA")),
...
dunn_df = filter(dunn, `dunn.P.adjusted` < 0.05) %>% 
  mutate("P Value" = sprintf("%.2e", (`dunn.P.adjusted`)), 
...
dunn_df2 = filter(dunn2, `dunn2.P.adjusted` < 0.05) %>% 
  mutate("P Value" = sprintf("%.2e", (`dunn2.P.adjusted`)), 
...
fish_data = read.csv("Fish_Species_V5.csv", 
                     na.strings = c("NA")) #load fish data
...
missing_percentage <- 100 * 
  sum(master_data$Collector == "MISSING") / nrow(master_data)
cat("Data with a Collector:", sprintf("%.2f%%", 100 - missing_percentage))
collector_data = filter(master_data, Collector != "MISSING", Size_Class != "0_05", 
...
ggplot(filter(summary_data2, Sum != 0), 
  ...
  geom_col(color = "black", width = 1, alpha = 0.8) + theme_classic() +

  labs(x = "Fish Species", y = "Observations") +
  theme(legend.position = "top", axis.text.x = element_text(angle = 90, face = "bold", size = 11, 
                                                            vjust = 0.5, hjust = 1),
...
loadings_df = data.frame(Loading_PC1 = (loadings_pc1),
                         Loading_PC2 = (loadings_pc2))
loadings_df = loadings_df %>%
  mutate(Sign_PC1 = ifelse(Loading_PC1 > 0, "pos", "neg"),
         Sign_PC2 = ifelse(Loading_PC2 > 0, "pos", "neg"))%>%
  mutate(Loading_PC1 = format(round(abs(Loading_PC1), 3), nsmall = 3),
